...
class VideoClipper:
    ...
    def clip(
        ...
        try:
            ...
            # 6. 最终拼接：封面 + 主视频 + 封底 + 黑屏（纯视频拼接）
            self._report("最终合成...", 95, progress_callback)
            self._final_concat(
                ffmpeg,
            ...
            # 7. 混入 BGM（循环到视频结束，音量 12%）
            self._report("混入背景音乐...", 97, progress_callback)
            bgm_path = str(_PROJECT_ROOT / "assets" / "audio" / "NIGHT DANCER - imase.mp3")
            if Path(bgm_path).exists():
                self._mix_bgm(ffmpeg, output_path, bgm_path)
            else:
                logger.warning(f"BGM 文件不存在，跳过混音: {bgm_path}")
    ...
    def _render_cover_image(self, resolution: Tuple[int, int], output_path: Path, has_mobile: bool = False):
        ...
        try:
            ...
            if bg_ratio > target_ratio:
                # 背景更宽：按高度缩放后左右裁剪
                new_h = h
                new_w = int(bg_raw.width * h / bg_raw.height)
            else:
                # 背景更高：按宽度缩放后上下裁剪
                new_w = w
                new_h = int(bg_raw.height * w / bg_raw.width)
            bg_resized = bg_raw.resize((new_w, new_h), Image.LANCZOS)
            left = (new_w - w) // 2
            top = (new_h - h) // 2
            bg = bg_resized.crop((left, top, left + w, top + h))
        except Exception:
    ...
    def _final_concat(
        ...
        """最终拼接：封面 + 主体 + 封底 + 黑屏（视频已带 BGM 音轨，无需混音）"""
    ...
    def _mix_bgm(self, ffmpeg: str, video_path: Path, bgm_path: str):
        """将 BGM 作为唯一音轨混入视频（循环播放到视频结束，音量 12%）。原视频有无音频均可。"""
        mixed_path = video_path.with_name(video_path.stem + "_bgm" + video_path.suffix)
        cmd = [
        ...
        proc = subprocess.run(cmd, capture_output=True, text=True, timeout=600)
        if proc.returncode != 0:
            logger.error(f"BGM 混音失败: {proc.stderr[-500:]}")
            raise RuntimeError(f"BGM 混音失败 (code={proc.returncode})")

        # 替换原文件
        os.replace(str(mixed_path), str(video_path))
        logger.info(f"BGM 混音完成: {video_path}")
    ...
    def _select_bgm_by_grade(self, grade: str) -> str:
        """（已废弃）录制已改为纯视频、剪辑阶段统一使用 NIGHT DANCER"""
        return ""
