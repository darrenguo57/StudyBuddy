"""
摄像头管理模块 - FFmpeg 子进程管道写入 H.264 MP4（含 BGM 音频轨 + 手机画中画合成）
"""
import logging
import subprocess
import time
...
class CameraManager:
    ...
    def __init__(self, config: CameraConfig = None, recording_dir: Path = None):
        ...
        # 录制队列与写线程（解耦采集与编码，避免 FFmpeg 管道阻塞预览）
        self._record_queue: Optional[queue.Queue] = None
        self._writer_thread: Optional[threading.Thread] = None
        self._writer_stop_event: Optional[threading.Event] = None

        # 手机端帧回调（由外部注入，用于画中画合成）
        self._mobile_frame_getter: Optional[Callable[[], Optional[np.ndarray]]] = None
        # 手机端连接状态回调
        self._mobile_connected_getter: Optional[Callable[[], bool]] = None
    ...
    def start_recording(self) -> Path:
        ...
        self._writer_thread = threading.Thread(
            ...
            args=(path, actual_w, actual_h, has_mobile),
            daemon=True,
        )
    ...
    def _writer_loop(self, path: Path, source_w: int, source_h: int, has_mobile: bool):
        """写线程：从队列取帧 → 合成画中画 → FFmpeg 管道写入纯视频 H.264 MP4（无音频）

        has_mobile=True: 手机竖屏为主画布，PC 画面为右下角 PiP
        has_mobile=False: PC 横屏为主画布（无合成）
        """
        import shutil

        # 自动发现 ffmpeg 路径：系统 PATH → imageio_ffmpeg 内置 → 常见位置
        ...
        if not ffmpeg_path:
            import os as _os
            candidates = [
                _os.path.join(str(Path(__file__).resolve().parents[2]), "tools", "ffmpeg.exe"),
            ...
            for c in candidates:
                if Path(c).exists():
                    ffmpeg_path = c
                    break
        ffmpeg_available = ffmpeg_path is not None and Path(ffmpeg_path).exists()
        if not ffmpeg_available:
            logger.warning(
                "FFmpeg 未安装或不在 PATH 中，将使用 OpenCV MJPG AVI 回退录制。"
                "安装 FFmpeg 并加入 PATH 可启用 H.264 MP4 录制。"
            )
        capture_fps = getattr(self, '_capture_fps', 5.0)
        input_fps = max(capture_fps, 3.0)

        # 确定画布尺寸（双重保险：has_mobile 已在上游校验过帧可用性，此处再次校验）
        if has_mobile:
            canvas_w, canvas_h = self._get_mobile_resolution()
            if canvas_w < 1 or canvas_h < 1:
                logger.warning("手机分辨率无效，回退为 PC 横屏模式")
                has_mobile = False
                canvas_w, canvas_h = source_w, source_h
            elif self._get_mobile_frame() is None:
                logger.warning("手机端帧已丢失，回退为 PC 横屏模式")
                has_mobile = False
                canvas_w, canvas_h = source_w, source_h
        else:
            canvas_w, canvas_h = source_w, source_h
        ...
        try:
            if ffmpeg_available:
                cmd = [
                    ...
                    '-s', f'{canvas_w}x{canvas_h}', '-pix_fmt', 'bgr24',
                    '-r', str(input_fps),
                    '-i', 'pipe:0',
                ...
                if proc is not None:
                    ...
                    if proc.poll() is not None:
                        ...
                        try:
                            ...
                            pass
                        logger.error(
                            f"FFmpeg 进程立即退出，返回码: {proc.returncode}"
                        )
                        if stderr_output:
                ...
                if proc is None:
                    # FFmpeg 启动失败，回退到 OpenCV MJPG AVI
                    ...
                    if not writer.isOpened():
                        raise RuntimeError(
                            f"VideoWriter 打开失败: {fallback_path} ({canvas_w}x{canvas_h})"
                        )
                    self.last_recording_path = fallback_path
                    logger.info(
                        f"VideoWriter 写线程已启动 (MJPG, 回退): {fallback_path} ({canvas_w}x{canvas_h})"
                    )
            else:
                # FFmpeg 未安装
                fallback_path = Path(str(path).replace('.mp4', '.avi'))
    ...
    def set_mobile_frame_getter(self, getter: Callable[[], Optional[np.ndarray]]):
        """注入手机帧获取回调（由外部调用，供画中画合成使用）"""
        self._mobile_frame_getter = getter

    def set_mobile_connected_getter(self, getter: Callable[[], bool]):
        """注入手机端连接状态回调（由外部调用，供分辨率决策使用）"""
        self._mobile_connected_getter = getter

    def _get_mobile_frame(self) -> Optional[np.ndarray]:
        """从共享缓冲获取最新手机帧"""
        try:
            if self._mobile_frame_getter is not None:
                return self._mobile_frame_getter()
        except Exception:
            pass
        try:
            from web.server import _mobile_frame_lock, _mobile_frame
            with _mobile_frame_lock:
                return _mobile_frame
        except Exception:
            return None

    def _check_mobile_connected(self) -> bool:
        """检查手机端是否已连接"""
        try:
            if self._mobile_connected_getter is not None:
                return self._mobile_connected_getter()
        except Exception:
            pass
        try:
            from web.server import _mobile_ws_connected, _mobile_frame_lock, _mobile_frame
            if _mobile_ws_connected:
                return True
            with _mobile_frame_lock:
                return _mobile_frame is not None
        except Exception:
            return False

    def _get_mobile_resolution(self) -> tuple:
        """获取手机端分辨率（宽, 高）。从共享缓冲取一帧获取尺寸"""
    ...
    def _compose_frame(self, pc_frame: np.ndarray) -> np.ndarray:
        ...
        # 首次检测到手机帧可用
        if not getattr(self, '_mobile_pip_logged', False):
            self._mobile_pip_logged = True
